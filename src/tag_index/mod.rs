...
pub struct Tag(pub Box<[u8]>);

/// a set of tags
pub type TagSet = FnvHashSet<Tag>;
...
pub struct DnfQueryBuilder {
    tags: FnvHashMap<Tag, u32>,
    sets: FnvHashMap<IndexSet, u32>,
}

impl DnfQueryBuilder {
    pub fn new() -> Self {
        Self::default()
    }

    /// Return the result as a [TagSetSet]
    pub(crate) fn tag_set_set(self) -> TagSetSet {
        let tags = self.tags;
